...
namespace K8sDashboard
{
    public class DashboardForm : Form
    {
        ...
        public DashboardForm(string ipAddress, string token)
        {
        ...
        private void IndexCapacityPerNode(JsonDocument nodesDoc)
        {
            ...
            foreach (var node in nodesDoc.RootElement.GetProperty("items").EnumerateArray())
            {
                ...
                string cpuStr = cap.GetProperty("cpu").GetString() ?? "0";
                string memStr = cap.GetProperty("memory").GetString() ?? string.Empty; 
